...
background-size: cover;
     ...
     <div id="media_panel" sytle="display:none;">
        <span id="quiz_audio_play_on"  style="display:none;"></span>
        <span id="quiz_video_play_on"  style="display:none;"></span>
        ...
        <div class="col-sm-9 col-sm-offset-3 col-md-10 col-md-offset-2 main">
          ...
          <div class="panel panel-default">
            <div class="panel-heading">
                <h3 class="panel-title" style="font-size:2em"></span>문제<small><span style="float:right;margin-right:20px;" class="glyphicon glyphicon-user" > 출제자:<span id="quiz_owner" > </span></span></small></h3>
            </div>
...
<script>
  ...
  function playAudio(play_audio, play_audio_file){
      
      
      var audioObject = document.getElementById('audio_player');
     var playFlag =$("#quiz_audio_play_on").html();
      
      if(play_audio == "YES"){
          $("#quiz_audio_play_on").html("NO");
          requestAjax("Ajax_media_control.php",debug_server_info, {control:"NO",type:"PLAY_AUDIO"});
          audioObject.src =play_audio_file;
          audioObject.load();
          audioObject.play();
      }else if(playFlag == "YES" && audioObject.ended ){
          audioObject.ended = false;
      }
  }
  function playVideo(play_video, play_video_file){
     
      var videoObject = document.getElementById('video_player');
      var playFlag =$("#quiz_video_play_on").html();
      
      if(play_video == "YES"){
          
          $("#quiz_video_play_on").html("NO");
          requestAjax("Ajax_media_control.php",debug_server_info, {control:"NO",type:"PLAY_VIDEO"});
          $("nav").hide();
          $(".container-fluid").hide()
          $("#media_panel").fadeIn();
          videoObject.src =play_video_file;
          videoObject.load();
          videoObject.play();
      }else if(playFlag == "YES" && videoObject.ended ){
          videoObject.ended = false;
      }else if(playFlag == "NO" && videoObject.ended ){
          videoObject.ended = false;
          $("nav").fadeIn();
  ...
  function current_quiz_info(list){
    ...
    for (var index in list){
      ...
      play_audio = list[index].play_audio;
      play_audio_file = list[index].play_audio_file;
      play_video = list[index].play_video;
      play_video_file = list[index].play_video_file;
    }
    $("#quiz_audio_play_on").html(play_audio);
    $("#quiz_video_play_on").html(play_video);
    
    playAudio(play_audio, play_audio_file);
    playVideo(play_video, play_video_file);
